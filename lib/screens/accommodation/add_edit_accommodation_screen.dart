...
class _AddEditAccommodationScreenState
  ...
  void _loadExistingAccommodation(List<Accommodation> accommodations) {
    ...
    final existing =
        accommodations.where((a) => a.id == widget.editAccommodationId);
    if (existing.isNotEmpty) {
      ...
      _depositPaidController.text =
          a.depositPaid > 0 ? a.depositPaid.toStringAsFixed(2) : '';
      _balanceDueController.text =
          a.balanceDue > 0 ? a.balanceDue.toStringAsFixed(2) : '';
  ...
  Future<void> _save() async {
    ...
    try {
      final notifier =
          ref.read(accommodationsProvider(widget.holidayId).notifier);
      if (_isEditing) {
  ...
  Widget build(BuildContext context) {
    final accommodationsAsync =
        ref.watch(accommodationsProvider(widget.holidayId));
              ...
              child: Form(
                ...
                _datePicker(
                  ...
                  onPick: () => _pickDate(
                    ...
                    onPicked: (v) => setState(() => _balanceDueDate = v),
                  ),
                ...
                _datePicker(
                  ...
                  onPick: () => _pickDate(
                    ...
                    onPicked: (v) => setState(() => _balancePaidDate = v),
                  ),
                  onClear: () => setState(() => _balancePaidDate = ''),
                ),
                const SizedBox(height: 20),

                // Confirmation Number
                Text('Confirmation Number', style: AppTextStyles.label),
                const SizedBox(height: 6),
                TextFormField(
